...
async def create_yookassa_payment(
    ...
    payload = {
        "amount" :{ # Сумма платежа
            "value": f"{amount:.2f}", # str(Decimal) - обязательная строка!
            "currency": "RUB",
        },
        "confirmation": { # Как подтвердить платеж
            "type" : "redirect", # Пользователь редиректируется на форму YooKassa
            ...
            #Но также мы можем передать и каждую позицию отдельно.
                {
                    ...
                    "payment_sybject": "commodity", # Тип: "service"=услуга, "commodity"=товар или заказ
                }
            ]
        }
    }
