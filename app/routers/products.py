...
async def get_all_products(
        ...
        categoty_id: int|None = Query(None, description="ID категории для фильтрации"),
    ...
    if categoty_id is not None:
        filters.append(ProductModel.category_id == categoty_id)
    ...
    total_smth = select(func.count()).select_from(ProductModel).where(*filters)
    ...
    if search:
        ...
        if search_value:
            ...
            total_smth = select(func.count()).select_from(ProductModel).where(*filters)
    total = await db.scalar(total_smth) or 0
    ...
    if rank_col is not None:
        ...
        result = await db.execute(products_smth)
        rows = result.all()
        ...
        items = (await db.scalars(product_smth)).all()
    return {
        "items": items,
        "total": total,
        "page": page,
...
async def create_product(
    ...
    """
    category_result = await db.scalars(select(CategoryModel).where(CategoryModel.is_active == True,
...
async def delete_product(
    ...
    await db.execute(update(ProductModel).where(ProductModel.id == product_id).values(image_url = None,
                                                                                      is_active=False)
    )
