...
from app.schemas import Order as OrderSchema, OrderList, OrderCheckouResponse
...
@router.post("/checkout", response_model=OrderCheckouResponse, status_code=status.HTTP_201_CREATED)
...
async def get_order_status(
    order_id: int,
    db:AsyncSession = Depends(get_async_db),
    curent_user: UserModel = Depends(get_current_user)
):
    order_smth = await db.scalars(select(OrderModel).where(OrderModel.id == order_id))
    order = order_smth.first()

    if not order:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Order not found")
    
    if order.user_id != curent_user.id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Order does not belong to an authorized user.")
